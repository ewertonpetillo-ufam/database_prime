...
-- Apenas os mesmos 11 medicamentos de produção / DRUG_OPTIONS em Step3DadosClinicos
INSERT INTO medications_reference (drug_name, generic_name, led_conversion_factor, medication_class) VALUES
    ('Levodopa / Carbidopa', NULL, 1.000, 'Levodopa'),
    ('Levodopa / Benserazida BD', NULL, 1.000, 'Levodopa'),
    ('Levodopa / Benserazida', NULL, 1.000, 'Levodopa'),
    ('Levodopa / Benserazida HBS', NULL, 0.750, 'Levodopa'),
    ('Levodopa / Benserazida DR', NULL, 0.750, 'Levodopa'),
    ('Levodopa / Carbidopa / Entacapona', NULL, 1.000, 'Levodopa'),
    ('Entacapone', NULL, 1.000, 'COMT Inhibitor'),
    ('Rasagilina', NULL, 1.000, 'MAO-B Inhibitor'),
    ('Safinamida', NULL, 1.000, 'MAO-B Inhibitor'),
    ('Amantadina', NULL, 1.000, 'NMDA Antagonist'),
    ('Pramipexol', NULL, 1.000, 'Dopamine Agonist');
...
CREATE TABLE patients (
    ...
    -- CPF em texto para consulta (cpf_hash mantido para anonimização)
    cpf VARCHAR(11),
    ...
    -- Health-related (voice/motor)
    deficiencia_visual BOOLEAN DEFAULT FALSE,
    hoarseness BOOLEAN DEFAULT FALSE,
    stuttering BOOLEAN DEFAULT FALSE,
...
COMMENT ON COLUMN patients.cpf IS 'CPF em texto para consulta (11 dígitos). cpf_hash mantido para anonimização.';
...
CREATE TABLE questionnaires (
    ...
    last_step INTEGER DEFAULT 1 CHECK (last_step >= 1 AND last_step <= 8),
...
CREATE UNIQUE INDEX uq_questionnaires_patient_id ON questionnaires(patient_id);
...
CREATE INDEX idx_questionnaires_id_patient ON questionnaires(id, patient_id);
...
CREATE INDEX idx_patient_medications_questionnaire ON patient_medications(questionnaire_id);
...
-- Parkinson's Disease Sleep Scale (PDSS-1) - Legacy table kept for historical data compatibility
CREATE TABLE pdss1_scores_legacy (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    questionnaire_id UUID UNIQUE NOT NULL REFERENCES questionnaires(id) ON DELETE CASCADE,

    -- 15 questions rated 0-10 (Visual Analogue Scale)
...
CREATE INDEX idx_pdss1_questionnaire_legacy ON pdss1_scores_legacy(questionnaire_id);
...
CREATE TABLE active_task_definitions (
    ...
    "group" INTEGER CHECK ("group" >= 1 AND "group" <= 5),
    description TEXT,
    instructions TEXT,
    active BOOLEAN DEFAULT TRUE
);
...
CREATE INDEX idx_task_def_stage ON active_task_definitions("group");
...
CREATE INDEX idx_binary_cpf_collected_at ON binary_collections(patient_cpf_hash, collected_at DESC);
CREATE INDEX idx_binary_questionnaire_collected_at ON binary_collections(questionnaire_id, collected_at DESC) WHERE (questionnaire_id IS NOT NULL);
CREATE INDEX idx_binary_questionnaire_or_cpf ON binary_collections(questionnaire_id, patient_cpf_hash) WHERE ((questionnaire_id IS NOT NULL) OR (patient_cpf_hash IS NOT NULL));
...
CREATE INDEX idx_pdf_reports_questionnaire ON pdf_reports(questionnaire_id);
...
-- Audit trail: versão otimizada para patients (log mínimo, melhor performance em UPDATE)
...
-- TA1: Mãos em repouso
INSERT INTO active_task_definitions (task_code, task_name, task_category, collection_form_type_id, "group", description, instructions, active)
VALUES (
    'TA1',
    'Mãos em repouso',
    'MOTOR',
